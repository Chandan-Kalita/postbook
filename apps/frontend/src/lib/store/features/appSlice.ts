...
export const appSlice = createAppSlice({
  ...
  reducers: (create) => ({
    setUser: create.reducer(
      (state, action: PayloadAction<{ user: IAppState['user'] }>) => {
    ...
    logoutAsync: create.asyncThunk(async (payload, thunkApi) => {
    ...
    login: create.asyncThunk(
      ...
      },
      {
        ...
        rejected: (state, action) => {
          ...
          alert(action.error || 'An error occurred');
        },
      }
    ),
...
export const selectAppLoading = (state: { app: IAppState }) =>
  state.app.loading;
